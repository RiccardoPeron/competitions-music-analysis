...
def slice_files(audiofilenames_full, slices_number):
    ...
    j = 0
    #slices_number = 3
    pbar = tqdm(audiofilenames_full, unit='files',
    ...
    for f in pbar:
        ...
        for i in range(3):
            ...
            slice_filename = os.path.join(
                DIR_SPLITTED_WAV_FILES, global_index + '-' + f + '_00' + str(i) + '.wav')
            sf.write(slice_filename, audio_data, sr, subtype='PCM_24')

    audiofilenames = [f for f in listdir(DIR_SPLITTED_WAV_FILES) if (
        os.path.isfile(os.path.join(DIR_SPLITTED_WAV_FILES, f)) and f.endswith(".wav"))]
...
def extract_files():
    ...
    for folder in folders:
        for f in sorted(listdir(DIR_WAV_FILES + '/' + folder)):
            if(os.path.isfile(os.path.join(DIR_WAV_FILES, folder, f)) and (f.endswith(".wav") or f.endswith(".mp3"))):
